...
export default function BudgetStrip({ rows }: { rows: Row[] }) {
  if (rows.length === 0) {
    return (
        ...
        {rows.map((r) => {
          ...
          return (
            <Link
              key={r.category}
              href="/settings/budgets"
              className="block snap-start shrink-0 w-44 rounded-xl bg-card ring-1 ring-foreground/10 p-3 cursor-pointer hover:ring-foreground/20 transition-all"
            >
